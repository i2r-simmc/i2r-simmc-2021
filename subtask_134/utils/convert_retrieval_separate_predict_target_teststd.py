...
def convert_json_to_flattened(
    ...
    output_path_src_retrieval=None,
    input_path_special_tokens="",
    output_path_special_tokens="",
    output_path_dialog_turn_id=""
):
    """
    ...
    if input_path_retrieval is not None:
        ...
        retrieval_targets= []
        retrieval_predicts = []
        # retrieval_groundtrue=[]
        start_index_retrieval=0
        dialog_turn_id_data=[]
    ...
    for _, dialog in enumerate(data):
        ...
        dialog_id_data = {"dialog_id":dialog_id,
                          "turn_info":[],
                          "final_turn_id":None,
                          "turns_disambiguation_label":[]}
        prev_asst_uttr = None
        prev_turn = None
        lst_context = []
        ...
        for turn_id, turn in enumerate(dialog[FIELDNAME_DIALOG]):
            ...
            if use_belief_states:
                ...
                if format_retrieval_options:
                    if turn_id==options_dict[dialog_id]["retrieval_candidates"]['turn_idx']:
                        dialog_id_data["final_turn_id"]=turn_id
                        turn_options = options_dict[dialog_id]["retrieval_candidates"]
                        # turn_options = (
                        #     options_dict[dialog_id]["retrieval_candidates"]
                        # )
                        count_options=0
                        for option_ind in turn_options["retrieval_candidates"]:
                            ...
                            retrieval_targets.append(options_pool[domain][option_ind])
                            retrieval_predicts.append(context)
                            count_options+=1
                        # retrieval_groundtrue.append([start_index_retrieval, start_index_retrieval+count_options, turn_options["gt_index"]])
                        dialog_id_data["turn_info"].append({"turn_id": turn_id,"flat_id":[start_index_retrieval, start_index_retrieval+count_options]})
                        start_index_retrieval += count_options
                    else:
                        dialog_id_data["turn_info"].append({"turn_id": turn_id})
                    if "disambiguation_label" in turn:
                        dialog_id_data["turns_disambiguation_label"].append(turn_id)

        dialog_turn_id_data.append(dialog_id_data)
    ...
    if format_retrieval_options:
        # print(retrieval_targets)
        # print(output_path_retrieval)
        # input()
        # Create a directory if it does not exist
        ...
        with open(output_path_src_retrieval, "w") as file_id:
            file_id.write("\n".join(retrieval_predicts))
        # with open(output_path_ground_true, "w") as f_ground_true:
        #     json.dump(retrieval_groundtrue, f_ground_true)
        with open(output_path_dialog_turn_id, "w") as f_dialog_turn_id:
            json.dump(dialog_turn_id_data, f_dialog_turn_id)
    if output_path_special_tokens != "":
        # Create a directory if it does not exist
