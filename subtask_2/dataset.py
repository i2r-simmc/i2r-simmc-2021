...
class SelectionDataset(Dataset):
    ...
    def batchify_join_str(self, batch):
        if self.mode == 'cross':
            ...
            text_segment_ids_list_batch_tensor = [torch.tensor(t, dtype=torch.long) for t in text_segment_ids_list_batch]
            text_token_ids_list_batch_pad = pad_sequence(text_token_ids_list_batch_tensor, batch_first=True)
            text_input_masks_list_batch_pad = pad_sequence(text_input_masks_list_batch_tensor, batch_first=True)
            text_segment_ids_list_batch_pad = pad_sequence(text_segment_ids_list_batch_tensor, batch_first=True)
            labels_batch_tensor = [torch.tensor(t, dtype=torch.long) for t in labels_batch]
            labels_batch_pad = pad_sequence(labels_batch_tensor, batch_first=True, padding_value=-1)

            return text_token_ids_list_batch_pad, text_input_masks_list_batch_pad, text_segment_ids_list_batch_pad, labels_batch_pad
            ...
            responses_input_masks_list_batch_tensor = [torch.tensor(t, dtype=torch.long) for t in responses_input_masks_list_batch]
            responses_token_ids_list_batch_pad = pad_sequence(responses_token_ids_list_batch_tensor, batch_first=True)
            responses_input_masks_list_batch_pad = pad_sequence(responses_input_masks_list_batch_tensor, batch_first=True)
            labels_batch_tensor = [torch.tensor(t, dtype=torch.long) for t in labels_batch]
            labels_batch_pad = pad_sequence(labels_batch_tensor, batch_first=True, padding_value=-1)

            return contexts_token_ids_list_batch_tensor, contexts_input_masks_list_batch_tensor, \
                          responses_token_ids_list_batch_pad, responses_input_masks_list_batch_pad, labels_batch_pad
