...
def eval_running_model(dataloader, model, test=False):
    model.eval()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dot_product_outfname', default='dot_product_devtest.txt', type=str) 
    ...
    if args.eval:
        ...
        test_result, dot_products = eval_running_model(val_dataloader, model, test=True)
        with open(os.path.join(args.output_dir, args.dot_product_outfname), 'w') as outf:
            for row in dot_products:
                np.savetxt(outf, row, fmt='%.4f')
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                if (step + 1) % args.gradient_accumulation_steps == 0:
                    ...
                    if global_step and global_step % eval_freq == 0:
                        val_result, _ = eval_running_model(val_dataloader, model)
